def checkmate(board):
    try:
        ...
        # up-date ตรวจสอบ board
        def is_square(board):
            row_length = len(board[0])  # หาความยาวของแถวแรก
            for row in board:
                if len(row) != row_length:
                    return False  # ถ้าแถวใดแถวหนึ่งมีความยาวไม่เท่ากัน แสดงว่าไม่เป็นรูปสี่เหลี่ยมจัตุรัส
            return True  # ถ้าทุกแถวมีความยาวเท่ากัน แสดงว่าเป็นรูปสี่เหลี่ยมจัตุรัส

        # ตรวจสอบก่อนว่ากระดานเป็นรูปสี่เหลี่ยมจัตุรัส
